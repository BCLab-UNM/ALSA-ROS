...
int main(int argc, char **argv) {
    ...
    logPublisher = mNH.advertise<std_msgs::String>("/swarm_log", 1, true);
			...
			case STATE_MACHINE_TRANSFORM: {
				...
				//Otherwise, assign a new goal
				  ...
				  goalLocation.theta = goal_state.yaw;
                                  ...
                                  log_msg.data = " ("+boost::lexical_cast<std::string>(time(NULL))+")"
                                      + ": ALSA controller selected a new goal state: ("
                                      + boost::lexical_cast<std::string>(goal_state.x) + ", "
                                      + boost::lexical_cast<std::string>(goal_state.y) + ", "
                                      + boost::lexical_cast<std::string>(goal_state.yaw)
                                      + ")";
	...
	//if target has not previously been detected 
        ...
        if (!targetsCollected[message->tags.data[0]]) {
			...
			stateMachineState = STATE_MACHINE_TRANSFORM;
                        ...
                        // Notify the ALSA controller that a target has been found
                        time_t t = time(NULL); // Get current time
                        int id = targetDetected.data; // By convention choose the first tag detected in the image
                        
                        std_msgs::String log_msg;
                        log_msg.data = " ("+boost::lexical_cast<std::string>(t)+")"
                            + ": Recorded a target with the ALSA controller ("
                            + boost::lexical_cast<std::string>(id) + ", "
                            + boost::lexical_cast<std::string>(currentLocation.x) + ", "
                            + boost::lexical_cast<std::string>(currentLocation.y)
                            + ")";
                        logPublisher.publish(log_msg);
                        
                        alsa_controller.addDetectedTarget(t, currentLocation.x, currentLocation.y, id);
                        alsa_controller.updateStrategy();

                        log_msg.data = " ("+boost::lexical_cast<std::string>(t)+")"
                            + ": ALSA Status: mu="
                            + boost::lexical_cast<std::string>(alsa_controller.getMu()) + ", "
                            + " hopkins_index="
                            + boost::lexical_cast<std::string>(alsa_controller.getHopkinsIndex());
                        logPublisher.publish(log_msg);
